...
export default function DataTable<T extends Record<string, any>>({ 
  ...
  return (
    <div className="space-y-4 select-none">
          ...
          <thead>
            <tr className="border-b border-zinc-800 text-zinc-500 uppercase tracking-widest text-[9px] font-bold">
              {columns.map((col, idx) => (
                <th 
                  key={idx} 
                  className={`py-4.5 px-5 font-bold ${
          ...
          <tbody>
            {loading ? (
              ...
              paginatedData.map((row, rIdx) => (
                <tr key={rIdx} className="border-b border-zinc-800/40 last:border-0 hover:bg-white/[0.01] transition-colors">
              ...
              <tr>
                <td colSpan={columns.length} className="py-12 text-center text-zinc-500 font-semibold">
                  No records found.
                </td>
              </tr>
            )}
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex justify-between items-center text-xs text-zinc-500 font-semibold pt-1">
          <span>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1.5 rounded-lg border border-zinc-800 bg-zinc-950 text-zinc-400 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
            >
