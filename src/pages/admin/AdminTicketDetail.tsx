      ...
      <div className="grid md:grid-cols-3 gap-6">
              ...
              {messages.length === 0 ? (
                ...
                <div key={m.id} className={`rounded-lg p-4 ${m.isStaff ? "bg-primary/10 border border-primary/20" : "bg-muted"}`}>
                  <p className="text-xs font-medium text-muted-foreground mb-1">
                    {m.isStaff ? "Staff" : ticket.username || "User"} &middot; {m.createdAt ? new Date(m.createdAt).toLocaleString() : ""}
                  </p>
        ...
        <Card className="border-border h-fit">
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <Separator />
            <div className="flex justify-between"><span className="text-muted-foreground">User</span><span className="text-foreground">{ticket.email || ticket.username}</span></div>
