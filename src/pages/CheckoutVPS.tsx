...
import { Check, ChevronLeft, Server, Cpu, HardDrive, AlertCircle, ExternalLink, Wifi, Key } from "lucide-react"
...
interface Plan {
  ...
  image?: string; cpuCores?: number; memoryMb?: number; storageGb?: number
}

interface VPSNode {
  id: string; name: string; country: string; ping?: string
  availableRam?: number; availableDisk?: number; isOnline?: boolean; flag?: string
}

interface VPSImage {
  fingerprint: string; aliases: string[]; description: string; sizeBytes?: number
}
...
export default function CheckoutVPS() {
  ...
  const [images, setImages] = useState<VPSImage[]>([])
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/checkout/vps-config").then(r => r.json()),
      fetch("/api/checkout/config").then(r => r.json())
    ])
      .then(([vpsData, checkoutData]) => {
        if (vpsData.success) {
          setNodes(vpsData.nodes || [])
          setImages(vpsData.images || [])
        } else {
          setConfigError("Failed to load VPS configuration")
        }
        if (checkoutData.success) {
          setCycles(checkoutData.cycles || [])
          setAddons(checkoutData.addons || [])
        }
      })
  ...
  const handlePay = async () => {
    if (!user || !plan || !serverName.trim() || !nodeId || !osTemplate || !agreeTerms) return
    ...
    return (
      <div className="py-24 text-center">
        <div className="mx-auto max-w-md px-6">
          ...
          <Link to="/vps"><Button>View Plans</Button></Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="py-24">
      <div className="mx-auto max-w-6xl px-6">
        <Link to="/vps" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors">
        ...
        <div className="grid lg:grid-cols-7 gap-8 items-start">
          <div className="lg:col-span-4 space-y-6">
            <Card className="border-border">
              <CardHeader>
                <CardTitle className="text-base">VPS Configuration</CardTitle>
                <CardDescription>Configure your virtual private server</CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                ...
                <div>
                  ...
                  {configError ? (
                    ...
                    <div className="grid grid-cols-1 gap-2 mt-1">
                      {nodes.filter(n => n.isOnline !== false).map(node => (
                        <label
                          key={node.id}
                        ...
                        >
                          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                            String(nodeId) === String(node.id) ? "border-primary" : "border-zinc-600"
                          }`}>
                            {String(nodeId) === String(node.id) && (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 text-xs text-muted-foreground mt-0.5">
                              {node.ping && <span>{node.ping} ping</span>}
                              {node.availableRam !== undefined && <span>{Math.round(node.availableRam / 1024)}GB RAM free</span>}
                            </div>
                          </div>
                        </label>
                      ))}
                ...
                <div>
                  ...
                  <Select value={osTemplate} onValueChange={setOsTemplate}>
                    ...
                    <SelectContent className="min-w-[200px]">
                      {images.map(img => {
                        const label = img.description || img.fingerprint || img.aliases?.[0] || img.osTemplate
                        return (
                          <SelectItem key={img.fingerprint || img.aliases?.[0] || img.osTemplate} value={img.fingerprint || img.aliases?.[0] || img.osTemplate}>
                            {label}
                          </SelectItem>
                        )
                      })}
                ...
                <div>
                  <Label htmlFor="root-password">Root Password (optional)</Label>
                  <Input
                    id="root-password"
                    type="password"
              ...
              <CardContent className="space-y-4">
                ...
                {!user ? (
                  ...
                  <Button
                    ...
                    onClick={handlePay}
                    disabled={processing || !serverName.trim() || !nodeId || !osTemplate || !agreeTerms}
                  >
                    {processing ? (
                      <>Processing...</>
                    ) : (
