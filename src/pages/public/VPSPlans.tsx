    ...
    <div className="py-24">
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">High-performance virtual servers with full root access.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {VPS_PRICING.map((plan) => (
            <Card key={plan.id} className={cn("card-hover border-border relative flex flex-col", plan.popular && "border-primary/50")}>
              ...
              <div className="px-6 pb-4 space-y-2.5 flex-1">
                ...
                <div className="flex items-center gap-2.5 text-sm pt-1 border-t border-border mt-3">
                  ...
                  <span className="text-foreground font-medium ml-auto">₹{plan.price}/${plan.usdPrice}</span>
                </div>
              </div>
              <div className="px-6 pb-6 mt-auto">
                <Link to={`/checkout/vps/${plan.id}`}>
